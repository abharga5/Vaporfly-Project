...
#omits na values
newdata_women <- na.omit(df_women)

#assign id's for each runner
newdata_women <- transform(newdata_women,nameid=as.numeric(factor(match_name)))
newdata_women <- transform(newdata_women,marathonid=as.numeric(factor(marathon)))
#add binary vaporfly yes or no
vaporbinary <- newdata_women$vaporfly*1
newdata_women <- cbind(newdata_women, vaporbinary)

#MODEL 1 (Varying intercepts and slopes)
#start mcmc process
Y <- newdata_women$time_minutes
v <- newdata_women$vaporbinary
m_id<-newdata_women$marathonid
n_id<-newdata_women$nameid
X <- cbind(v,m_id,n_id)
M <- 21
J <- 370
...
data     <- list(Y=Y,n=778, v=v, J=J, M=M,
...
samples4 <- coda.samples(model, variable.names=params,
...
jags_out <- rjags::coda.samples(model,
                                variable.names=c("beta","Sigma.u","Sigma.w","Sigma_e"),
                                n.iter=30000, progress.bar="none")
